...
window.addEventListener('message', async (event) => {
  if (event.data.type !== 'EXTRACT_LIBBY_BOOK') return;
  ...
  try {
    ...
    if (!window.__odreadCmptParams) {
      window.parent.postMessage({
        ...
        error: 'Crypto parameters not captured yet. Play the audiobook for a few seconds and try again.'
      }, '*');
      return;
    }
    ...
    for (const spine of BIF.objects.spool.components) {
      const url = location.origin + '/' + spine.meta.path + '?' + odreadCmptParams[spine.spinePosition];
    ...
    console.log(`[Libby Downloader] Extraction complete: "${bookData.metadata.title}" (${chapters.length} chapters)`);
